...
const benefitKeys = ["expertise", "flexibility", "cost", "vision"] as const;
const missionKeys = ["governance", "riskManagement", "compliance", "operational"] as const;

export default function RSSITempsPartage() {
  const t = useTranslations("conseilsAudits.ciso");

  const formulas = t.raw("formulas.list") as Array<{ name: string; days: string; desc: string }>;
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-lg max-w-none text-[#3c3a47]">
            ...
            <div className="bg-[#7d53de]/10 border-l-4 border-[#7d53de] p-6 rounded-r-lg">
              <p className="text-[#0e0c19] font-semibold mb-2">{t("intro.operationalTitle")}</p>
              <p>{t("intro.operationalText")}</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefitKeys.map((key) => (
              <div key={key} className="bg-white p-6 rounded-xl shadow-lg">
                <h3 className="text-lg font-bold text-[#7d53de] mb-2">{t(`benefits.${key}.title`)}</h3>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {missionKeys.map((key) => {
              const items = t.raw(`missions.${key}.items`) as string[];
              return (
