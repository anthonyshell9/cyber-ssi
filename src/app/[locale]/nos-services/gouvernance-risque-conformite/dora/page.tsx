...
export default function DORA() {
  const t = useTranslations("grc.dora");

  const entities = t.raw("entities.list") as string[];
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-lg max-w-none text-[#3c3a47]">
            ...
            <div className="bg-[#7d53de]/10 border-l-4 border-[#7d53de] p-6 rounded-r-lg">
              <p className="text-[#0e0c19] font-semibold mb-2">{t("intro.obligationsTitle")}</p>
              <ul className="list-disc list-inside space-y-2">
                {(t.raw("intro.obligations") as string[]).map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {pillarKeys.map((key) => {
              const items = t.raw(`pillars.${key}.items`) as string[];
              return (
                <div key={key} className="bg-gray-50 p-8 rounded-xl">
                  <h3 className="text-xl font-bold text-[#0e0c19] mb-2">
                    {t(`pillars.${key}.title`)}
                  </h3>
                  <p className="text-[#7d53de] text-sm mb-4">{t(`pillars.${key}.description`)}</p>
          ...
          <div className="mt-8 bg-gradient-to-br from-[#0e0c19] to-[#161131] p-8 rounded-xl text-center">
            ...
            <p className="text-[#7d53de] text-sm mb-4">
              {t("pillars.pillar5.description")}
            </p>
            <p className="text-white/80">
              {t("pillars.pillar5.text")}
            </p>
          </div>
        </div>
      </section>
