...
const serviceKeys = ["entra", "m365", "azureSec", "intune"] as const;

export default function SecuriteAzure() {
  const t = useTranslations("servicesOperationnels.azure");

  const features = t.raw("features.list") as string[];
  const approachSteps = t.raw("approach.steps") as Array<{ step: string; title: string; description: string }>;
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-lg max-w-none text-[#3c3a47]">
            ...
            <div className="bg-[#7d53de]/10 border-l-4 border-[#7d53de] p-6 rounded-r-lg">
              <p className="text-[#0e0c19] font-semibold mb-2">{t("intro.sharedTitle")}</p>
              <p>{t("intro.sharedText")}</p>
            </div>
          </div>
        </div>
      </section>
